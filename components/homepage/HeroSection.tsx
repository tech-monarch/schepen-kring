    ...
    <AuthGuard requireAuth={false}>
          ...
          <div className="lg:w-1/2 p-6 lg:p-10 flex flex-col justify-center">
            <div className="mb-4">
              {mode !== "login" && (
                <div className="flex bg-gray-100 p-1 rounded-lg mb-4">
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${mode === "register" ? "bg-white shadow text-[#003566]" : "text-gray-400"}`}
                  >
                    USER
                  </button>
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-bold rounded-md transition-all ${mode === "partner" ? "bg-white shadow text-[#003566]" : "text-gray-400"}`}
                  >
                    PARTNER
                  </button>
                </div>
              ...
              <p className="text-xs text-gray-500">
                {mode === "login"
                  ? "Please enter your terminal credentials."
                  : "Register to access the fleet management system."}
              </p>
            </div>

            <form className="space-y-4" onSubmit={handleSubmitTrigger}>
              {error && (
                <div className="bg-red-50 border border-red-200 text-red-600 px-3 py-2 rounded-lg text-[13px] font-medium">
                  {error}
                </div>
              )}
              {success && (
                <div className="bg-green-50 border border-green-200 text-green-600 px-3 py-2 rounded-lg text-[13px] font-medium">
                  {success}
                </div>
              )}
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="w-3.5 h-3.5 text-[#003566] border-gray-300 rounded focus:ring-[#003566] cursor-pointer"
                    />
                    <label
                      htmlFor="rememberMe"
                  ...
                  <Link
                    ...
                    className="text-xs text-gray-600 hover:text-[#003566]"
                  >
                    {t("forgotPassword")}
                  </Link>
                </div>
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full bg-[#003566] hover:bg-[#001d3d] text-white font-semibold py-2.5 rounded-lg mt-2 transition-all text-sm"
              >
                {isLoading
                  ? "Processing..."
                  : mode === "login"
      ...
      <Dialog open={showTerms} onOpenChange={setShowTerms}>
        <DialogContent className="sm:max-w-[400px] bg-white">
          <DialogHeader>
            <DialogTitle className="text-[#003566] font-bold">
              Yacht Brokerage Protocol
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-3 py-2 text-[10px] text-gray-600 uppercase tracking-widest leading-relaxed">
            <p>
              1. Partner agrees to maintain high standards of yacht brokerage
              transparency.
            </p>
            <p>
              2. Security notice: System automatically logs IP address and
              device fingerprints for terminal security.
            </p>
            <p>
              3. Access is strictly limited until full verification is completed
              in account setup.
            </p>
          </div>
          <Button
            onClick={executeAuth}
            className="bg-[#003566] text-white text-xs uppercase font-bold"
          >
            I APPROVE & REGISTER
          </Button>
        </DialogContent>
