...
export function ReferralModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const [stats, setStats] = useState({ my_code: "", invite_link: "", friends_joined: 0, bonuses_earned: 0 });
  ...
  const fetchInviteStats = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/invite/stats`, {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none bg-white rounded-[32px]">
        ...
        <div className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-blue-600 p-8 text-center text-white">
          ...
          <DialogTitle className="text-2xl font-black tracking-tight">Invite & Earn €10</DialogTitle>
          <p className="text-indigo-100 text-xs mt-2 font-medium">
            Share the magic of Answer24 with your friends!
          </p>
        </div>
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-2">
            <p className="text-slate-500 text-xs font-medium text-center px-4 leading-relaxed">
              When a friend signs up via your link and books a deal for <span className="text-slate-900 font-bold">2+ people</span>, you both get <span className="text-emerald-500 font-black">€10.00</span> in your wallet.
            </p>
          </div>
          ...
          <div className="bg-slate-50 rounded-[24px] p-4 border border-slate-100 space-y-3">
            <div className="flex items-center justify-between px-1">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Personal Invite Link</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50/50 rounded-2xl p-4 border border-slate-100 text-center">
              <p className="text-2xl font-black text-slate-900">{stats.friends_joined}</p>
              <p className="text-[10px] font-black text-slate-400 uppercase">Friends Joined</p>
            </div>
            <div className="bg-emerald-50/50 rounded-2xl p-4 border border-emerald-100 text-center">
              <p className="text-2xl font-black text-emerald-600">€{stats.bonuses_earned}</p>
              <p className="text-[10px] font-black text-emerald-500 uppercase">Total Earned</p>
            </div>
          </div>
