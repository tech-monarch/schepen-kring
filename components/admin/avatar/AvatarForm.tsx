...
const AvatarForm: React.FC<AvatarFormProps> = ({ avatar }) => {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validImageTypes.includes(file.type))
      return setError(
        "Invalid image type. Please select a JPG, PNG, or GIF file.",
      );
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || "An unexpected error occurred while saving the avatar.";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <Card className="shadow-sm hover:shadow-md transition-shadow">
            <CardHeader>
              ...
              <CardDescription>
                Provide the details for your new avatar.
              </CardDescription>
            </CardHeader>
                  ...
                  {formData.functions.map((func, index) => (
                    <div key={index} className="flex items-center space-x-3">
                      <Input
                        ...
                        value={func}
                        onChange={(e) =>
                          handleFunctionChange(index, e.target.value)
                        }
        ...
        <div className="space-y-8">
          <Card className="shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-6">
              <div>
                <Label htmlFor="required_plan" className="font-semibold">
                  Required Plan
                </Label>
                <Select
