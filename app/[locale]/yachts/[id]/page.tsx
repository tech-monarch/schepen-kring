...
export default function YachtTerminalPage() {
  ...
  const renderList = (text?: string) => {
    if (!text) return <span className="text-slate-400 italic">No equipment listed.</span>;
    return (
            ...
            <div className="relative h-[60vh] overflow-hidden group bg-slate-200">
              ...
              <div className="absolute bottom-6 left-6 right-6 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                <Thumbnail
                  ...
                  onClick={() => setActiveImage(yacht.main_image ? `${STORAGE_URL}${yacht.main_image}` : PLACEHOLDER_IMAGE)}
                />
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                  ...
                  <div className="space-y-6">
                     <div className="flex items-center gap-2 text-blue-600 border-b border-blue-100 pb-2 mb-4">
                        <Ship size={16} /> 
                        <span className="text-[10px] font-black uppercase tracking-widest">Hull & General</span>
                     </div>
                  ...
                  <div className="space-y-6">
                     <div className="flex items-center gap-2 text-blue-600 border-b border-blue-100 pb-2 mb-4">
                        <Zap size={16} /> 
                        <span className="text-[10px] font-black uppercase tracking-widest">Engine Room</span>
                     </div>
                     <div className="grid grid-cols-2 gap-y-4">
                  ...
                  <div className="space-y-6">
                     <div className="flex items-center gap-2 text-blue-600 border-b border-blue-100 pb-2 mb-4">
                        <Bed size={16} /> 
                        <span className="text-[10px] font-black uppercase tracking-widest">Accommodation</span>
                     </div>
                     <div className="grid grid-cols-2 gap-y-4">
                        ...
                        <SpecRow label="Water System" value={yacht.water_capacity} /> {/* Fallback if needed */}
                     </div>
                  </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="space-y-4">
                        <div>
                           <p className="text-[9px] font-bold uppercase text-slate-400 mb-1">Navigation & Electronics</p>
                           {renderList(yacht.navigation_electronics)}
                        </div>
                        <div>
                           <p className="text-[9px] font-bold uppercase text-slate-400 mb-1">Exterior & Deck</p>
          ...
          <div className="lg:col-span-4 p-8 md:p-12 flex flex-col gap-8 bg-white sticky top-20 h-fit border-l border-slate-50">
            ...
            <div className="bg-slate-50 p-6 border border-slate-100 rounded-sm">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={placeBid} className="bg-[#003566] hover:bg-blue-900 h-auto px-6">
                  <Gavel size={16} />
                </Button>
              </div>
            </div>
            ...
            <div className="border-2 border-[#003566] p-6 rounded-sm relative overflow-hidden">
              ...
              <p className="text-[9px] font-black uppercase tracking-widest text-[#003566] mb-1">
                Asking Price {yacht.vat_status && <span className="text-slate-400">({yacht.vat_status})</span>}
              </p>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400">
                <span>Location</span>
                <span className="text-[#003566] flex items-center gap-1"><MapPin size={10} /> {yacht.location}</span>
              </div>
              <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400">
                <span>Length</span>
            ...
            <div className="bg-slate-50 p-6 rounded-sm border border-slate-100 mt-4">
                ...
                <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                  {bids.length > 0 ? (
                    bids.map((bid: any, i) => (
                      <div key={bid.id} className={cn("flex justify-between items-center text-xs", i===0 ? "text-[#003566] font-bold" : "text-slate-400")}>
                        <span>{bid.user?.name || "Private Collector"}</span>
                        <span>€{Number(bid.amount).toLocaleString()}</span>
                      </div>
                    ))
                  ) : (
                    <p className="text-[10px] text-slate-400 italic">No bids recorded yet.</p>
                  )}
                </div>
              </div>
          </div>
            ...
            >
              ...
              {paymentStatus === "processing" && (
                <div className="py-20 text-center flex flex-col items-center">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Contacting Secure Gateway...</p>
                </div>
              )}
              {paymentStatus === "success" && (
                <div className="py-12 text-center">
                  ...
                  <h3 className="text-xl font-serif mb-2 text-[#003566]">Transaction Secured</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
...
function Thumbnail({ src, active, onClick }: { src: string; active: boolean; onClick: () => void; }) {
  ...
  return (
    ...
    >
      <img src={src} onError={handleThumbError} className="w-full h-full object-cover" alt="Thumbnail" />
    </button>
  );
}
