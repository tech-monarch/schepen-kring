...
export default function RoleManagementPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [uRes, pRes] = await Promise.all([
        ...
        axios
          .get(`${API_BASE}/permissions`, getHeaders())
          .catch(() => ({ data: [] })),
      ]);
      setUsers(uRes.data);
  ...
  const togglePermission = async (userId: number, permName: string) => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE}/users/${userId}/toggle-permission`,
        { permission: permName },
        getHeaders(),
      );
      toast.success(
        `${permName.replace(/_/g, " ")} ${res.data.status === "attached" ? "Granted" : "Revoked"}`,
        {
          style: {
            fontSize: "10px",
            fontWeight: "bold",
  ...
  const terminateAccount = async (id: number) => {
    if (
      !confirm(
        "TERMINATE ACCOUNT: This will permanently remove this user from the fleet. Proceed?",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-100 pb-10">
        ...
        <div className="flex w-full md:w-auto gap-4">
          ...
          <Button
            ...
            className="bg-[#003566] text-white rounded-none h-12 px-8 uppercase text-[10px] tracking-widest font-black shadow-xl hover:bg-blue-900 transition-all"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border-2 border-dashed border-slate-100">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              No personnel found in manifest
            </p>
          </div>
        ) : (
            ...
            >
              <div className="flex flex-col md:flex-row justify-between gap-8">
                ...
                <div className="w-full md:w-2/3 md:border-l border-slate-50 md:pl-8">
                  <p className="text-[8px] uppercase tracking-widest text-slate-400 mb-4 font-black">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-[#003566]/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white p-10 border border-slate-200 w-full max-w-lg shadow-2xl relative"
            >
              <button
              ...
              <form onSubmit={handleCreateUser} className="space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <Key
                      size={14}
                      className="absolute right-6 top-2 text-slate-300"
                    />
                ...
                <Button
                  type="submit"
                  className="w-full bg-[#003566] text-white rounded-none h-14 uppercase text-[10px] tracking-[0.3em] font-black shadow-lg hover:bg-blue-900"
                >
                  Finalize Enrollment
                </Button>
              </form>
