  ...
  const impersonateUser = async (userId: number) => {
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/users/${userId}/impersonate`,
        {},
        getHeaders(),
      );
      ...
      toast.success(`Identity assumed: ${res.data.user.name}`, {
        id: "impersonate",
      });
      setTimeout(() => {
  ...
  return (
    <div className="space-y-10 p-6 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-100">
        {(["Employee", "Admin", "Partner", "Customer"] as UserCategory[]).map(
          (tab) => (
            <button
              key={tab}
            ...
            >
              ...
              {activeTab === tab && (
                <motion.div
                  layoutId="activeTab"
                  className="absolute bottom-0 left-0 right-0 h-1 bg-[#003566]"
                />
              )}
            </button>
          ),
      ...
      <div className="grid grid-cols-1 gap-6 pb-20">
        {loading ? (
          <Loader2
            className="animate-spin mx-auto mt-20 text-slate-200"
            size={48}
          />
                ...
                {user.role === "Employee" && pagePermissions.length > 0 && (
                  <div className="lg:w-2/3 lg:border-l border-slate-100 lg:pl-10">
                    <div className="flex justify-between items-center mb-6">
                      <div>
                        <p className="text-[8px] font-black uppercase text-slate-400 tracking-[0.3em]">
                          Page Access Control
                        </p>
                    ...
                    <div className="space-y-4">
                      {pagePermissions.map((page) => (
                        ...
                        >
                          <div>
                            <p className="text-[10px] font-bold uppercase text-[#003566]">
                              {page.page_name}
                            </p>
                            {page.description && (
                            ...
                            <div className="text-center">
                              <div
                                className={cn(
                                  ...
                                  getPermissionValue(user.id, page.page_key) ===
                                    0
