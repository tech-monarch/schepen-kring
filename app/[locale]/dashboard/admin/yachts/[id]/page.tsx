...
import {
  Camera, Loader2, Upload, Waves, Coins, Images, Trash, AlertCircle,
  Ship, Compass, Box, CheckSquare, Sparkles, CheckCircle, Zap, Bed, Save, ArrowLeft
...
// Configuration
const STORAGE_URL = "https://kring.answer24.nl/storage/"; // [cite_start]// [cite: 4]
const PLACEHOLDER_IMAGE = "https://images.unsplash.com/photo-1569263979104-865ab7cd8d13?auto=format&fit=crop&w=600&q=80"; // [cite: 4]
...
export default function YachtEditorPage() {
  ...
  useEffect(() => {
    ...
    const fetchYachtDetails = async () => {
      try {
        ...
        // Populate Main Image
        setMainPreview(yacht.main_image ? `${STORAGE_URL}${yacht.main_image}` : null); // [cite: 22]
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(isNewMode ? "Vessel Registered Successfully" : "Manifest Updated Successfully");
        ...
        <form onSubmit={handleSubmit} className="space-y-16">
            ...
            <div className="space-y-4">
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-slate-200 p-6 flex justify-between items-center z-50">
                  ...
                  >
                    {isSubmitting ? <Loader2 className="animate-spin mr-2 w-5 h-5" /> : <Save className="mr-2 w-5 h-5" />}
                    {isNewMode ? "Initialize Vessel Registry" : "Save Manifest Changes"}
                  </Button>
              </div>
        </form>
      </div>
...
function Label({ children }: { children: React.ReactNode }) {
    return <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 pl-0.5">{children}</p>; // [cite: 172]
}
