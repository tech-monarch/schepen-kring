  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-12 text-[#003566]">
      ...
      <div className="relative mb-10 group">
        <Search
          className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600"
          size={20}
        />
        <input
          type="text"
                ...
                <div className="p-8 space-y-4 flex-1">
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-4 border-t border-slate-50 text-[9px] font-black uppercase text-slate-400">
                    <div className="flex items-center gap-1">
                      <Maximize2 size={12} /> {yacht.length}
                    </div>
                    <div className="flex items-center gap-1">
                      <Calendar size={12} /> {yacht.year}
                    </div>
                    <div className="flex items-center gap-1">
                      <MapPin size={12} /> {yacht.location || "Local"}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <SheetContent className="sm:max-w-[950px] bg-[#F8FAFC] p-0 border-l-12 border-[#003566] overflow-y-auto">
          ...
          >
            <div className="bg-[#003566] p-10 text-white sticky top-0 z-40 flex justify-between items-center shadow-xl">
              <div>
                <SheetTitle className="text-4xl font-serif italic">
                  Vessel Manifest Entry
                </SheetTitle>
                <p className="text-blue-400 text-[9px] font-black uppercase tracking-[0.4em]">
                  Registry Auth: Admin
                </p>
              </div>
              <X
            ...
            <div className="p-10 space-y-16">
              ...
              <div className="space-y-4">
                ...
                <div
                  ...
                  className="h-96 bg-white border-2 border-dashed border-slate-200 relative flex items-center justify-center cursor-pointer overflow-hidden shadow-inner group"
                >
                  <input
                    id="main_image_input"
                    name="main_image"
              ...
              <div className="bg-white p-10 border border-slate-200 shadow-sm space-y-10">
                <h3 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em] flex items-center gap-2 border-b border-slate-50 pb-6 italic">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {[
                    ...
                    <div key={f.name} className="space-y-2">
                      <label className="text-[9px] font-black uppercase text-[#003566]">
                        {f.label}
                      </label>
                      <input
                        name={f.name}
                        ...
                        className="w-full bg-slate-50 p-4 border-b-2 border-slate-100 text-[#003566] font-bold text-sm outline-none focus:border-blue-600 transition-all shadow-inner"
                        required={f.req}
                      />
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <select
                      name="status"
                      defaultValue={selectedYacht?.status || "Active"}
                      className="w-full bg-slate-50 p-4 border-b-2 border-slate-100 text-[#003566] font-black uppercase text-[10px] outline-none appearance-none"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 p-4 border-b-2 border-slate-100 text-[#003566] font-bold text-sm outline-none focus:border-blue-600"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#003566]/5 p-10 border border-blue-100 space-y-10">
                <h3 className="text-[10px] font-black text-[#003566] uppercase tracking-[0.4em] flex items-center gap-2 italic border-b border-blue-100 pb-6">
                  <Waves size={16} /> Technical Specifications Matrix
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  {[
                    ...
                    <div key={f.name} className="space-y-2">
                      <label className="text-[8px] font-black text-blue-600 uppercase tracking-widest">
                        {f.label}
                      </label>
                      <input
                        name={f.name}
                        type={f.type || "text"}
                        defaultValue={selectedYacht?.[f.name]}
                        className="w-full bg-white border-b-2 border-slate-100 p-3 text-[11px] font-bold text-[#003566] outline-none focus:border-blue-600"
                      />
                    </div>
                  ))}
                </div>
