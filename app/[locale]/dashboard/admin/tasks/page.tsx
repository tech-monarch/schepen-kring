...
import { 
  Plus, User as UserIcon, X, Trash2, Search, Loader2, Anchor
} from "lucide-react";
...
export default function AdminTaskBoardPage() {
  ...
  const handleCreateTask = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormData({ title: "", priority: "Medium", status: "To Do", assigned_to: "", yacht_id: "", due_date: "" });
    } catch (err) {
  ...
  const toggleTaskStatus = async (id: number, currentStatus: string) => {
    ...
    try {
      await axios.patch(`${API_BASE}/tasks/${id}/status`, { status: nextStatus }, getHeaders()); // [cite: 80]
    } catch (err) {
    ...
    <div className="space-y-10 p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-widest text-blue-600 font-black mt-2">Fleet Management & Command</p>
        </div>
        ...
        <div className="flex w-full md:w-auto gap-4">
          ...
          <Button onClick={() => setIsModalOpen(true)} className="bg-[#003566] text-white rounded-none h-12 px-8 uppercase text-[10px] tracking-widest font-black shadow-lg">
      ...
      <div className="bg-white border border-slate-200 divide-y divide-slate-100 shadow-sm">
        {loading && tasks.length === 0 ? (
          ...
          filteredTasks.map((task) => (
            <div key={task.id} className="p-6 flex flex-col lg:flex-row justify-between items-center gap-6 group hover:bg-slate-50/50 transition-all">
              <div className="flex gap-6 items-start w-full lg:w-auto">
                <div className={cn(
                  "w-1 h-12 shrink-0", 
                  (task.priority === "Urgent" || task.priority === "High") ? "bg-red-500" : "bg-blue-600"
                )} />
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-[#003566] uppercase tracking-widest">{task.title}</h3>
                  </div>
                  <div className="flex items-center gap-4 text-[9px] uppercase tracking-widest text-slate-400 mt-2 font-medium">
                    <span className="flex items-center gap-1.5 text-slate-600 font-bold">
                      <UserIcon size={12} className="text-blue-600" /> {task.assigned_to?.name || "Unassigned"}
                    </span>
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={cn(
                    ...
                    task.status === "Done" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-white text-slate-500 border-slate-200"
                  )}
                >
                  {task.status}
                </button>
                <button onClick={() => deleteTask(task.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 transition-all">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 bg-[#003566]/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white p-8 border border-slate-200 w-full max-w-md shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-serif italic text-[#003566]">Assign New Task</h2>
                <X className="cursor-pointer text-slate-400" onClick={() => setIsModalOpen(false)} />
              </div>
              
              <form onSubmit={handleCreateTask} className="space-y-6">
                <div className="space-y-1">
                  <label className="text-[8px] font-black uppercase tracking-widest text-slate-400">Task Title</label>
                  <input 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1">
                    <label className="text-[8px] font-black uppercase tracking-widest text-slate-400">Crew Member</label>
                    <select 
                    ...
                    >
                      ...
                      {staffOnly.map(u => <option key={u.id} value={u.id}>{u.name} — {u.role}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black uppercase tracking-widest text-slate-400">Target Vessel</label>
                    <select 
                      className="w-full border-b border-slate-200 py-2 outline-none text-[10px] uppercase font-bold"
                ...
                <Button type="submit" className="w-full bg-[#003566] text-white rounded-none h-12 uppercase text-[10px] tracking-widest font-black">
                  Deploy Assignment
                </Button>
              </form>
            </motion.div>
