      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 bg-white border border-slate-200 p-8 shadow-sm">
          <div className="flex justify-between items-center mb-8 border-b border-slate-50 pb-4">
            ...
            <Link
              href="/nl/dashboard/admin/yachts"
          ...
          <div className="space-y-4">
            {data.recentBids.map((yacht: any, i) => (
              <div
                key={yacht.id}
                className="flex items-center justify-between p-4 bg-slate-50/50 border border-slate-100 hover:bg-white hover:border-blue-200 transition-all group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-[#003566] uppercase tracking-wider">
                      {yacht.name}
                    </p>
                    <p className="text-[9px] text-slate-400 uppercase tracking-tighter font-medium">
                      Registry: {yacht.vessel_id}
                    </p>
                  </div>
                </div>
            ...
            {!loading && data.recentBids.length === 0 && (
              <div className="py-20 text-center space-y-2">
                <p className="text-[10px] uppercase text-slate-300 tracking-[0.2em]">
                  No active bidding protocols
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white border border-slate-200 p-8 shadow-sm">
          <h2 className="text-xs uppercase tracking-[0.3em] font-black text-[#003566] italic mb-8 border-b border-slate-50 pb-4">
            System Audit
          </h2>
              ...
              <div key={task.id} className="flex gap-4 items-start group">
                ...
                <div>
                  <p className="text-[9px] text-slate-500 leading-relaxed uppercase tracking-widest font-medium">
                    <span className="text-[#003566] font-bold">
                      {task.assigned_to ? "Operator" : "Auto-System"}
                    </span>{" "}
                    updated
                    <span className="text-blue-600 font-black italic mx-1">
                      "{task.title}"
                    </span>
