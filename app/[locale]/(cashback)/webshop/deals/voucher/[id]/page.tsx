...
export default function VoucherPage() {
  ...
  if (error || !deal) return (
    <div className="h-screen flex flex-col items-center justify-center gap-4">
      ...
      <button onClick={() => router.push('/webshop')} className="text-blue-600 hover:underline">Return to Webshop</button>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 flex flex-col items-center justify-center print:bg-white print:p-0">
      ...
      <div className="mb-8 flex gap-4 print:hidden">
        <button onClick={() => window.print()} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all">
          <Printer className="w-5 h-5" /> Print / Save as PDF
        </button>
        <button onClick={() => router.back()} className="flex items-center gap-2 bg-white text-slate-600 px-6 py-3 rounded-xl font-bold shadow-sm hover:bg-slate-50 transition-all">
      ...
      <div className="w-full max-w-[450px] bg-white border-[1.5px] border-slate-100 rounded-[40px] relative overflow-hidden print:border-none print:shadow-none print:rounded-none">
        ...
        <div className="p-10 text-left border-b-2 border-dashed border-slate-100">
           <img src={deal.deal_image || 'https://api.answer24.nl/placeholder.png'} className="w-16 h-16 rounded-[20px] object-cover mb-6 shadow-sm" alt="Logo" />
           <h1 className="text-[28px] font-[800] text-slate-900 tracking-tight leading-none mb-2">{deal.deal_name}</h1>
           <p className="text-sm font-medium text-slate-500">Reservation Confirmation & Voucher</p>
        </div>

        {/* Details Section */}
        <div className="p-10">
          <div className="flex items-center gap-2 mb-6 pb-2 border-b border-slate-100">
             <span className="text-[10px] font-[800] uppercase tracking-[0.15em] text-slate-400">Booking Details</span>
          </div>

          <div className="grid grid-cols-2 gap-6">
            <div className="mb-2">
               <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Customer</p>
               <p className="text-sm font-[600] text-slate-700 break-words">{deal.customer_email}</p>
            </div>
            <div className="mb-2 text-right">
               <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Arrival Date</p>
               <p className="text-sm font-[600] text-slate-700">
                 {new Date(deal.booking_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
               </p>
            </div>
            <div className="mb-2">
               <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Guests</p>
               <p className="text-sm font-[600] text-slate-700">{deal.people_count || '1'} Person(s)</p>
            </div>
            <div className="mb-2 text-right">
               <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Reference</p>
               <p className="text-sm font-[600] text-slate-700">A24-{deal.id.toString().slice(-4).toUpperCase()}</p>
            </div>
          </div>

          <div className="mt-8 bg-slate-50 rounded-3xl p-5">
             <div className="flex gap-2 text-[11px] text-slate-500 mb-2">
               <span>•</span><span>Please present this voucher upon arrival.</span>
             </div>
             <div className="flex gap-2 text-[11px] text-slate-500">
               <span>•</span><span>Valid only for the specified date.</span>
             </div>
          </div>
        </div>
        ...
        <div className="mx-10 p-6 bg-slate-900 rounded-3xl flex justify-between items-center text-white">
           <div>
             <p className="text-[11px] font-bold opacity-60 uppercase">Total Amount</p>
             <p className="text-2xl font-[800] tracking-tight">€{deal.price || deal.deal_price || '0.00'}</p>
           </div>
        ...
        <div className="p-10 text-center">
           <div className="inline-block p-3 border border-slate-100 rounded-3xl mb-5">
              ...
              <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="#0f172a" strokeWidth="1.5">
      ...
      <style jsx global>{`
        @media print {
          ...
          .min-h-screen { height: auto; display: block; padding: 20px; }
          /* Center content in print */
          .w-full.max-w-\\[450px\\] { margin: 0 auto; width: 100%; border: none; }
        }
      `}</style>
    </div>
  );
