...
export default function DealDetailPage() {
  ...
  const handleMollieTopUpAndPay = async (
    ...
    const initPage = async () => {
      try {
        ...
        if (dealsResult.success) {
          ...
          const found = dealsResult.data.find(
        ...
        if (token && user) {
          const walletRes = await fetch(
            `${API_BASE}/admin/ledger/balance/${user.mainId || user.id}`,
            {
    ...
    const initPage = async () => {
      try {
        ...
        const res = await fetch(
          `https://api.answer24.nl/api/v1/merchant/deals`,
        );
        ...
        if (token && userId) {
          const walletRes = await fetch(
            `https://api.answer24.nl/api/v1/admin/ledger/balance/${userId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/json",
    ...
    const fetchWallet = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.answer24.nl/api/v1/wallet/balance`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    try {
      ...
      // SCENARIO 2: Partial Wallet + Mollie Top-up
        ...
        if (depositData.checkoutUrl) {
          toast.info(
            `Saldo onvoldoende. U wordt doorgeleid naar Mollie voor het restant van €${gapAmount.toFixed(2)}`,
          );
          ...
          throw new Error(
            depositData.message || "Kon betaling niet initialiseren.",
          );
        }
      }
    } catch (error: any) {
...
`);
  ...
  if (!selectedItem)
    return <div className="p-20 text-center">Deal niet gevonden.</div>;
        ...
        <div className="w-full flex justify-center mt-16 px-4 md:px-6">
          <div className="w-full max-w-7xl h-[35vh] max-h-[400px] min-h-[250px] grid grid-cols-4 grid-rows-2 gap-2 overflow-hidden rounded-4xl shadow-sm bg-gray-50">
            ...
            {[...Array(4 - sideImages.length)].map((_: any, i: number) => (
              <div
                key={`filler-${i}`}
                className="col-span-1 row-span-1 bg-gray-100 overflow-hidden opacity-40"
              >
                <img
                  src={
              ...
              {paymentStep === 1 && (
                <div className="space-y-6">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] font-black text-blue-200 uppercase">
                        Totaal Bedrag
                      </p>
                      <p className="text-2xl font-[1000] text-green-300">
                        €{totalAmount}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-white/10 rounded-3xl p-5 border border-white/10">
                      <label className="text-[10px] font-black uppercase text-blue-200 block mb-2">
                        E-mailadres
                      </label>
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          type="email"
                          readOnly
                          className="bg-transparent outline-none text-white w-full font-bold opacity-80"
                          value={customerEmail}
                        />
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-7 gap-1.5 p-3 bg-white rounded-3xl">
                        {getNextMonthDates().map((date) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`text-[6px] font-black uppercase ${isSelected ? "text-white/60" : "text-blue-400"}`}
                              >
                                {date.dayName}
                              </span>
              ...
              {paymentStep === 2 && (
                <div className="space-y-8 py-4">
                  ...
                  <button
                    ...
                    className="w-full bg-white text-blue-600 py-5 rounded-2xl font-black text-lg shadow-xl flex items-center justify-center gap-3"
                  >
                    {isProcessing ? (
                  ...
                  <button
                    onClick={() => setPaymentStep(1)}
                    className="w-full text-[10px] font-black text-white/40 uppercase tracking-[0.3em] text-center"
                  >
                    Annuleren en terug
                  </button>
                </div>
                ...
                <div className="space-y-8 py-6">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-blue-100 font-medium mb-8">
                      Your booking is confirmed. Your voucher is ready below.
                    </p>
                  </div>
                      ...
                      <button
                        onClick={() =>
                          handleDownloadTicket({
                            ...
                            deal_name: selectedItem.name,
                            deal_image:
                              selectedItem.image_url || selectedItem.logo,
                  ...
                  <div className="bg-blue-900/40 rounded-2xl p-5 border border-white/10 flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-blue-100 leading-relaxed">
                        ...
                        time ({bookingTime}). Show your digital or printed
                        voucher at the entrance.
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div className="border-t border-slate-100 bg-slate-50/50 mt-20 py-24 w-full">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex items-center gap-4 mb-16">
            ...
            <div>
              <span className="text-blue-600 font-black text-[10px] uppercase tracking-[0.4em] block mb-1">
                More to explore
              </span>
              <h2 className="text-4xl md:text-5xl font-[1000] text-slate-900 uppercase italic tracking-tighter">
                Latest Updates
              </h2>
            </div>
          </div>
          ...
          <div className="mb-24">
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                onClick={() => router.push("/nl/webshop/deals")}
              ...
              {allDeals
                ...
                .map((item: any) => (
                  <motion.div
                    ...
                    onClick={() =>
                      router.push(
                        `/webshop/deals/${item.id}/${item.slug || "view"}`,
                      )
                    }
                    whileHover={{ y: -8 }}
                      ...
                      <div className="mb-8 pt-4 border-t border-slate-50">
                        ...
                        <div className="flex gap-1.5">
                          {["1", "2", "3", "4", "5", "6", "7"].map((dayNum) => {
                            const isActive =
                              item.available_days?.includes(dayNum);
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              navigator.clipboard.writeText(
                                `${window.location.origin}/nl/webshop/deals/${item.id}/${item.slug || "view"}`,
                              );
                              toast.success("Link gekopieerd!");
                            }}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {blogs.map((blog) => (
                <div
                  key={blog.id}
                  onClick={() => router.push(`nl/blog/${blog.slug}`)}
                  className="group block bg-white rounded-[2rem] overflow-hidden border border-slate-200 p-3 cursor-pointer"
                >
