...
import { Search, Tag, ShoppingBag, X, Check, Loader2, Mail, ArrowRight, Users, Sparkles, Calendar, Globe, Bell, Download, Ticket } from "lucide-react";
...
// }, [activeMainTab]);
  ...
  useEffect(() => {
    const handleHash = () => {
      ...
      if (hash === '#webshop' || hash === '#webshops') setActiveMainTab("webshops");
          ...
          >
            ...
            <div className="max-w-lg mx-auto relative group">
              ...
              <input
                ...
                placeholder={activeMainTab === "webshops" 
...
{activeMainTab === "requested" ? (
        ...
        >
          <div className="flex items-center gap-5 mb-6">
            <div className="w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center overflow-hidden">
              {deal.deal_image ? (
                <img src={deal.deal_image} className="w-full h-full object-cover" alt="" />
              ) : (
          ...
          <div className="space-y-3 border-t border-slate-50 pt-6">
            <div className="flex items-center gap-3 text-slate-500">
              ...
              <span className="text-xs font-bold">
                {new Date(deal.booking_date).toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' })}
              </span>
            </div>
            <div className="flex items-center gap-3 text-slate-500">
              <Mail className="w-4 h-4" />
              <span className="text-xs font-bold truncate">{deal.customer_email}</span>
            </div>
          </div>
          <div className="flex gap-2 mt-6 pt-4 border-t border-slate-50">
            <button 
              onClick={() => { setSelectedItem(deal); setPaymentStep(3); }}
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
    {filteredItems.map((item) => (
      <motion.div key={item.id} layoutId={item.id} onClick={() => { setSelectedItem(item); setPaymentStep(1); }}
        whileHover={{ y: -8 }}
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
               ...
               <span className="text-2xl font-[1000] text-slate-900">
                  {item.type === 'webshop' ? item.cashback : `€${item.deal_price}`}
               </span>
            </div>
                ...
                <div className="lg:sticky lg:top-10 h-fit space-y-6">
                  {selectedItem.type === 'webshop' ? (
                    <div className="bg-blue-600 rounded-[2rem] p-8 text-white shadow-2xl shadow-blue-200 space-y-6">
                      ...
                      <p className="text-blue-100 font-medium text-sm opacity-90 leading-relaxed">Proceed to the merchant's domain to initiate your transaction and secure your cashback allocation.</p>
                      <a href={selectedItem.url} target="_blank" rel="noopener noreferrer"
                        ...
                        Visit Website <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                      </a>
                    </div>
                  ) : (
                    <>
                        ...
                        {paymentStep === 1 && (
                          <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="bg-white/10 rounded-[1.5rem] p-5 border border-white/10">
                                <label className="text-[10px] font-black uppercase tracking-widest text-blue-200 block mb-2 opacity-80">Select Date & Time</label>
                                <div className="flex items-center gap-3">
                                  <Calendar className="w-5 h-5 text-blue-300" />
                                  <input type="datetime-local" className="bg-transparent border-none outline-none text-white w-full font-bold [color-scheme:dark]" value={bookingDate} onChange={(e) => setBookingDate(e.target.value)} />
                                </div>
                              </div>
                            </div>
                        ...
                        {paymentStep === 2 && (
                          <div className="animate-in fade-in zoom-in-95 duration-500 text-center">
                             <div className="bg-white/10 rounded-2xl p-6 mb-8 border border-white/10 text-left space-y-4">
                                <div>
                                   <p className="text-blue-100 text-[10px] uppercase font-black mb-1 opacity-70">Booking for</p>
                                    <p className="text-white font-bold">{customerEmail}</p>
                                </div>
                                <div>
                                    <p className="text-blue-100 text-[10px] uppercase font-black mb-1 opacity-70">Party Size</p>
                                    <p className="text-white font-bold">{peopleCount} {peopleCount === 1 ? 'Person' : 'Guests'}</p>
                                </div>
                                <div>
                                    <p className="text-blue-100 text-[10px] uppercase font-black mb-1 opacity-70">Appointment</p>
                                    <p className="text-white font-bold">
                                    {bookingDate ? new Date(bookingDate).toLocaleString([], { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute:'2-digit' }) : "Not set"}
                                    </p>
                                </div>
                             </div>
                             <button onClick={handleBookDeal} className="w-full bg-white text-blue-600 py-5 rounded-2xl font-black shadow-lg flex items-center justify-center mb-5 transition-all active:scale-95">
                                {isProcessing ? <Loader2 className="animate-spin" /> : "Confirm & Send Ticket"}
                             </button>
  ...
  <div className="py-4">
    ...
    <div className="text-center mb-8">
      ...
      <h4 className="text-2xl font-[1000] text-white tracking-tight">Booking Confirmed</h4>
      <p className="text-blue-100 text-xs opacity-70 uppercase tracking-widest font-bold mt-1">Ticket ID: #RD-{selectedItem.id.toString().slice(-5)}</p>
    </div>
    ...
    <div className="bg-white rounded-3xl p-6 text-slate-900 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 mb-6 pb-6 border-b border-dashed border-slate-200">
        ...
        />
        <div>
          <h5 className="font-black text-lg leading-tight">{selectedItem.deal_name || selectedItem.name}</h5>
          <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">{selectedItem.category || 'Member Deal'}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-y-4 gap-x-8 mb-6">
        <div>
          ...
          <p className="text-sm font-black text-slate-800">
             {new Date(selectedItem.booking_date || bookingDate).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}
          </p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Guests</p>
          <p className="text-sm font-black text-slate-800">{selectedItem.people_count || peopleCount} Persons</p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Price</p>
          <p className="text-sm font-black text-blue-600">€{selectedItem.price || selectedItem.deal_price || '0.00'}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Status</p>
          <p className="text-sm font-black text-green-500 uppercase">{selectedItem.status || 'Active'}</p>
        </div>
      </div>
      ...
      <div className="bg-slate-50 rounded-2xl p-4 flex items-center justify-between border border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] font-bold text-slate-600 truncate max-w-[150px]">
            {selectedItem.customer_email || customerEmail}
          </span>
        </div>
        <div className="px-3 py-1 bg-white border border-slate-200 rounded-lg shadow-sm">
           <span className="text-[10px] font-black">VALID</span>
        </div>
      </div>
    </div>
