...
import { Search, Tag, ShoppingBag, X, Check, Loader2, Mail, ArrowRight, Users, Sparkles, Calendar, Globe, Bell, Download, Ticket, ChevronLeft, ChevronRight, Clock, MapPin, Info,ArrowLeft  } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-6 -mt-8 relative z-20 pb-32">
      ...
      <div className="flex flex-col gap-6 mb-12">
        <div className="flex bg-white/80 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200/60 shadow-lg self-center md:self-start">
          <button onClick={() => { window.location.hash = "deals"; setActiveMainTab("deals"); }}
          ...
          <button onClick={() => setActiveMainTab("requested")}
      ...
      {activeMainTab === "requested" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {requestedDeals.map((deal: RequestedDeal) => (
            <motion.div key={deal.id} className="bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm">
          ...
          {filteredItems.map((item: any) => (
            <motion.div key={item.id} layoutId={item.id} onClick={() => handleCardClick(item)}
              ...
              <div className="relative h-64 overflow-hidden">
                <img src={item.image_url || item.logo} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              ...
              <div className="p-8">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{item.category} • Eindhoven</p>
                <h3 className="text-xl font-black text-slate-900 mb-4 line-clamp-1">{item.name}</h3>
                
                <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                  <div>
                    <span className="text-slate-300 line-through text-xs font-bold block">€{(item.deal_price * 1.6).toFixed(2)}</span>
                    <span className="text-3xl font-[1000] text-slate-900">€{item.deal_price}</span>
                  </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-white overflow-y-auto">
          ...
          <div className="sticky top-0 z-[120] bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4">
            <div className="max-w-7xl mx-auto flex justify-between items-center">
              <button onClick={() => setSelectedItem(null)} className="flex items-center gap-2 font-black text-[10px] uppercase tracking-widest text-slate-900 bg-slate-100 px-5 py-2.5 rounded-full hover:bg-slate-200 transition-all">
                <ArrowLeft className="w-4 h-4" /> Terug naar overzicht
              </button>
              <div className="flex gap-2">
                <button onClick={copyDealLink} className="p-3 bg-slate-50 rounded-full hover:text-blue-600 transition-all"><Globe className="w-5 h-5" /></button>
                <button onClick={() => setSelectedItem(null)} className="p-3 bg-slate-50 rounded-full"><X className="w-5 h-5" /></button>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-8">
              ...
              <div className="lg:col-span-2 space-y-10 px-6 md:px-0">
                <div className="relative aspect-video rounded-[3rem] overflow-hidden bg-slate-100 shadow-2xl">
                  <img src={selectedItem.image_url || selectedItem.logo} className="w-full h-full object-cover" alt="" />
                </div>

                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="bg-blue-100 text-blue-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase">Populair in Eindhoven</span>
                    <div className="flex items-center gap-1 text-yellow-400"><Sparkles className="w-3 h-3 fill-current" /><span className="text-slate-900 font-black text-xs">4.9 (500+ reviews)</span></div>
                  </div>
                  <h1 className="text-4xl md:text-6xl font-[1000] text-slate-900 leading-tight tracking-tighter">
                    {selectedItem.name}
                  </h1>
                  <p className="flex items-center gap-2 text-slate-500 font-bold"><MapPin className="w-4 h-4" /> Eindhoven Center</p>
                </div>

                {/* Highlights Card (Core Social Deal UX) */}
                <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm">
                  <h2 className="text-xl font-black mb-8 flex items-center gap-3">
                    <div className="w-1.5 h-6 bg-blue-600 rounded-full" /> Highlights
                  </h2>
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {["Exclusief 3-gangen keuzediner", "Sfeervolle locatie", "Geldig op alle dagen", "Vers bereide gerechten"].map((h, i) => (
                      <li key={i} className="flex gap-4 items-start">
                        <div className="mt-1 bg-green-100 p-1.5 rounded-full"><Check className="w-3.5 h-3.5 text-green-600" /></div>
                ...
                <div className="lg:sticky lg:top-28 space-y-6">
                  <div className="bg-white rounded-[3rem] p-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-slate-100">
                    <div className="flex justify-between items-end mb-8">
                      <div>
                        <span className="text-slate-300 line-through text-sm font-bold block">€{(selectedItem.deal_price * 1.6).toFixed(2)}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-5xl font-[1000] text-slate-900 tracking-tighter">€{selectedItem.deal_price}</span>
                          <span className="bg-green-500 text-white px-3 py-1 rounded-xl text-xs font-black">-39%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {paymentStep === 1 ? (
                      <div className="space-y-6">
                        <div className="space-y-4">
                          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 focus-within:border-blue-500 transition-all">
                            ...
                            <input type="email" placeholder="naam@mail.nl" className="bg-transparent w-full outline-none font-bold text-slate-900" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} />
                          </div>
                          
                          <div className="grid grid-cols-2 gap-3">
                             {/* Simplified Guests Selector */}
                             <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                                <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Personen</label>
                                <input type="number" min="1" className="bg-transparent w-full outline-none font-bold text-slate-900" value={peopleCount} onChange={(e) => setPeopleCount(parseInt(e.target.value))} />
                             </div>
                             <button onClick={() => document.getElementById('cal')?.scrollIntoView({behavior: 'smooth'})} className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-left">
                                <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Datum</label>
                                <span className="text-xs font-bold text-blue-600">{bookingDate || 'Selecteer'}</span>
                             </button>
                          </div>
                        </div>

                        <button onClick={() => setPaymentStep(2)} disabled={!customerEmail || !bookingDate} 
                        ...
                        <div id="cal" className="pt-4 border-t border-slate-100">
                          <p className="text-[10px] font-black uppercase text-slate-400 mb-4">Kies een datum</p>
                          <div className="grid grid-cols-4 gap-2 h-48 overflow-y-auto no-scrollbar">
                             {getNextMonthDates().map((date) => (
                                <button key={date.fullDate} disabled={!date.isAvailable} onClick={() => setBookingDate(date.fullDate)}
                                  ...
                                  <span className="text-[8px] font-black uppercase">{date.dayName}</span>
                                  <span className="text-sm font-black">{date.dayNumber}</span>
                                </button>
                             ))}
                          </div>
                        </div>
                      ...
                      /* Final Review Step */
                      <div className="space-y-4">
                         <div className="bg-blue-50 p-4 rounded-2xl text-blue-600 text-sm font-bold">Totaal te betalen: €{selectedItem.deal_price}</div>
                         <button onClick={handleBookDeal} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg">
                           {isProcessing ? <Loader2 className="animate-spin mx-auto" /> : "Bevestig & Betaal"}
                         </button>
                         <button onClick={() => setPaymentStep(1)} className="w-full text-slate-400 font-bold text-sm">Wijzig gegevens</button>
                      </div>
                    )}
                  </div>
                  
                  <div className="bg-slate-50 rounded-[2rem] p-6 border border-slate-100">
                    <h4 className="font-black text-xs uppercase tracking-widest mb-4">Voorwaarden</h4>
                    <ul className="space-y-3">
                      <li className="flex gap-2 text-[11px] font-bold text-slate-500"><Clock className="w-4 h-4 text-blue-500" /> Geldig tot 3 maanden na aankoop</li>
                      <li className="flex gap-2 text-[11px] font-bold text-slate-500"><Info className="w-4 h-4 text-blue-500" /> Vooraf reserveren verplicht</li>
                    </ul>
                  </div>
                </div>
              </div>
