...
import { Search, Tag, ShoppingBag, X, Check, Loader2, Mail, ArrowRight, Users, Sparkles, Calendar, Globe, Bell, Download, Ticket, ChevronLeft, ChevronRight, Clock, Link } from "lucide-react";
...
//   // 1. Calculate TOTAL price based on number of guests
...
//   // 2. Check if user has enough money for the TOTAL
//   if (currentBalance < totalToPay) {
//     toast.error(`Onvoldoende saldo. Totaal is €${totalToPay.toFixed(2)}, je komt €${(totalToPay - currentBalance).toFixed(2)} tekort.`);
//     return;
//   }
...
//     if (res.ok) {
//       setPaymentStep(3);
//       // 3. Locally deduct the TOTAL amount
//       setWalletBalance(prev => prev - totalToPay);
//       toast.success("Succesvol betaald met wallet!");
//     } else {
                      ...
                      {paymentStep === 3 && (
                        <div className="py-4">
                          <div className="text-center mb-8">
                            <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white/10"><Check className="w-8 h-8 text-white" /></div>
                          ...
                          <div className="bg-white rounded-3xl p-6 text-slate-900 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-dashed border-slate-200">
                              <img src={selectedItem.deal_image || selectedItem.logo || selectedItem.image_url} className="w-14 h-14 rounded-xl object-cover shadow-sm" alt="" />
                              <div><h5 className="font-black text-lg leading-tight">{selectedItem.deal_name || selectedItem.name}</h5><span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">{selectedItem.category || 'Member Deal'}</span></div>
                            </div>
                            <div className="grid grid-cols-2 gap-y-4 gap-x-8 mb-6">
                              <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Date</p><p className="text-sm font-black text-slate-800">{new Date(selectedItem.booking_date || bookingDate).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}</p></div>
                              <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Guests</p><p className="text-sm font-black text-slate-800">{selectedItem.people_count || peopleCount} Persons</p></div>
                              <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Price</p><p className="text-sm font-black text-blue-600">€{selectedItem.price || selectedItem.deal_price || '0.00'}</p></div>
                              <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Status</p><p className="text-sm font-black text-green-500 uppercase">{selectedItem.status || 'Active'}</p></div>
                            </div>
                            <div className="bg-slate-50 rounded-2xl p-4 flex items-center justify-between border border-slate-100">
                              <div className="flex items-center gap-3"><Mail className="w-4 h-4 text-slate-400" /><span className="text-[11px] font-bold text-slate-600 truncate max-w-[150px]">{selectedItem.customer_email || customerEmail}</span></div>
                              <div className="px-3 py-1 bg-white border border-slate-200 rounded-lg shadow-sm"><span className="text-[10px] font-black">VALID</span></div>
                            </div>
                          </div>
                          <div className="mt-8 space-y-3">
                            <button onClick={() => handleDownloadTicket(selectedItem)} className="w-full bg-white text-blue-600 py-4 rounded-2xl font-black text-sm transition-all shadow-lg flex items-center justify-center gap-2 hover:bg-blue-50">
                              <Download className="w-4 h-4" /> Download PDF Voucher
                            </button>
                            <button onClick={() => setSelectedItem(null)} className="w-full bg-white/10 hover:bg-white/20 text-white py-4 rounded-2xl font-black text-sm transition-all">Close Ticket</button>
                          </div>
                        </div>
                      )}
                    </div>
